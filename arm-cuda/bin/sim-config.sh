...
#                 --allocation <account> --partition <partition>
#
...
if [ -n "$optionlist_file" ]; then
    ...
    # this may need to go through srun: --exec-prefix "srun -A <account> -p <partition> -n 1"
...
# Partition and allocation are optional and have no default. A site whose
# scheduler already picks a sensible default should get no -p/-A at all,
# rather than an empty one: `#SBATCH -p ` is a hard error, and inventing a
# default here would be wrong on every machine but the one it was copied
# from. Each is emitted only when asked for.
[ -n "$allocation" ] && alloc_flag=" -A @ALLOCATION@" || alloc_flag=""
[ -n "$partition" ]  && part_flag=" -p $partition"    || part_flag=""
[ -n "$allocation" ] && sb_alloc="#SBATCH -A @ALLOCATION@" || sb_alloc="# (no allocation configured; the scheduler default applies)"
[ -n "$partition" ]  && sb_part="#SBATCH -p @QUEUE@"       || sb_part="# (no partition configured; the scheduler default applies)"
[ -n "$partition" ]  && ini_queue="queue           = $partition" || ini_queue="# queue         = (unset: the scheduler's default partition is used)"
[ -n "$allocation" ] && ini_alloc="allocation      = $allocation" || ini_alloc="# allocation    = (unset)"
