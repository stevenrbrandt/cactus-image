...
SYS_LIBDIRS="/usr/lib/aarch64-linux-gnu /usr/lib /usr/lib64"
...
for flavor in openmpi mpich; do
    ...
    libdir="/usr/lib/aarch64-linux-gnu/hdf5/$flavor"
    ok=1
...
# Anything an optionlist names via a *DIR/*DIRS key must actually exist.
# This catches a cfg pointing at a path that isn't in the image (e.g.
# copying another distro's layout, where MPICH's libs live somewhere else).
# The pattern deliberately does not require an underscore before DIR(S), so
# that Cactus's un-prefixed LIBDIRS key is covered as well as MPI_INC_DIRS
# and friends.
for cfg in /opt/cactus-deps/*.cfg; do
    ...
    done < <(grep -E '^[A-Z_0-9]+(DIR|DIRS)[[:space:]]*=' "$cfg")
